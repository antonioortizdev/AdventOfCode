...
def get_fields(entry: str):
    x = int(entry[0:entry.index('-')])
    y = int(entry[entry.index('-')+1:entry.index(' ')])
    letter = entry[entry.index(':')-1:entry.index(':')]
    password = entry[entry.index(':')+1:].strip()
    return x, y, letter, password

# entry is valid if 'password' has between 'x' and 'y' 'letter'
...
for entry in input:
    if is_valid(entry): valid_count += 1
    if is_valid2(entry): valid_count_2 +=1
print('part1', valid_count) # 548
print('part2', valid_count_2) # 502
